...
def output_directory(outputs):
    ...
    # 出力先ディレクトリの作成
    output_dir = outputs
    base_img_dir = Path(output_dir, "cifar")
    img_dir = base_img_dir

    # 既存の同名フォルダや(n)付きフォルダをチェック
    existing_dirs = [d for d in base_img_dir.parent.glob(f"{base_img_dir.name}([0-9]+)") if d.is_dir()]
    max_n = 0
    ...
    # 基本ディレクトリが既に存在する場合、(max_n + 1)から開始
    n = max_n + 1 if base_img_dir.exists() or existing_dirs else 0
    img_dir = base_img_dir if n == 0 else base_img_dir.parent / f"{base_img_dir.name}({n})"

    # ユニークなディレクトリ名を確保（(n)をインクリメントして存在しない名前を見つける）
    while img_dir.exists():
        n += 1
        img_dir = base_img_dir.parent / f"{base_img_dir.name}({n})"
